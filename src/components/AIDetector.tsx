  ...
  const analyzeText = async () => {
    ...
    try {
      ...
      const data = await callChatGptApi(
        `You are an expert AI content detection system. Analyze the provided text and determine whether it was likely written by a human or generated by an AI model (e.g. ChatGPT, Claude, Gemini).
Consider: sentence structure variety, natural flow, personal voice, hedging language, repetitive patterns, overly formal tone, and lack of authentic opinion.
Return ONLY a valid JSON object with these exact fields and nothing else:
{ "aiProbability": <integer 0-100>, "confidence": <integer 75-95>, "explanation": "<one or two sentences describing your reasoning>" }`,
        `Analyze this text for AI-generated content detection:\n\n${text}`
      );

      let parsed: { aiProbability: number; confidence: number; explanation: string } | null = null;
      ...
      if (parsed && typeof parsed.aiProbability === "number") {
        setResult({
          aiProbability: Math.min(100, Math.max(0, parsed.aiProbability)),
          confidence: Math.min(100, Math.max(60, parsed.confidence ?? 80)),
        ...
        setResult({
          ...
          confidence: 70,
          explanation:
            "The AI returned an unstructured response. The text shows mixed characteristics — review manually for best results.",
        });
      }
